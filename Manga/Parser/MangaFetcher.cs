...
namespace WeebLib.Manga.Parser
{
    public class MangaFetcher : IWebFetcher<MangaData>
    {
        ...
        protected override int Fetch(MangaData data, int start)
        {
            return new MangaScrapper().Scrape(new MangaSearcher().QueryBatoAndGetChapters(ref data, start), WorkDir);
        }

        public int Fetch(SearchType searchResults, int first, int amount)
        {
            return Fetch(new MangaData(searchResults.name, amount, searchResults.link, searchResults.source), first);
        }
